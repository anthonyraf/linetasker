...
from py_tasks.core import Register, ConfigFile, Task, Status
...
class TestConfigFile:
    ...
    @pytest.fixture
    def config_file(cls):
        return ConfigFile(cls.FILE_BASENAME, cls.CURRENT_DIR)

    @pytest.fixture
    def filename(self):
        return self.FILE_BASENAME + ".json"

    def test_filename_is_correct(self, config_file: ConfigFile, filename: str):
        assert config_file.filename == filename
    
    @pytest.mark.dependency(name="file_is_created")
    def test_file_is_created(self, config_file: ConfigFile, filename: str):
        config_file.create_file()

        assert os.path.exists( 
            self.CURRENT_DIR / Path(filename)
        )

    @pytest.mark.dependency(name="default_file_template",
                            depends=["file_is_created"])
    def test_default_file_template(self, config_file: ConfigFile, filename: str):
        default_json = {"tasks": [], 
                        "cursor": 0, 
                        "cumul_count": 0}

        with open(self.CURRENT_DIR / Path(filename), "r") as target:
            assert target.read() == json.dumps(default_json, indent=4)


    @pytest.mark.dependency(name="read_json",
                            depends=["default_file_template"])
    def test_read_json_default(self, config_file: ConfigFile):
